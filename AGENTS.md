...
## 4. 全中文呈现与自由思考规范 (Chinese Presentation Protocol - 最高优先级)

1. **深度思考过程自由化 (Flexible Thinking)**：
   - 模型在内部思考推理阶段（包括 `<thought>`、Thinking Process、Chain-of-Thought、自我反思、步骤规划与方案推演），**不强制限制思考语言**。模型可根据技术场景自由使用中文、英文或混合形式展开高强度深度推理，充分发挥最强逻辑与代码推理能力。
2. **用户展示界面零英文策略 (Zero-English User Presentation)**：
   - 所有展示给用户看的内容必须为纯正中文。对中文用户的所有交互对话、气泡消息、任务汇报（Handoff Report）、方案设计、代码解释、任务拆解与总结汇报一律采用简体中文，严禁向用户展示未经翻译的英文自然语言整句或混合杂交句式。
